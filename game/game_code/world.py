...
class Map(object):
        ...
        for cell in self.map:
            if cell is not None:
                ...
                for direction in neighbour:
                    ...
                    barrier_pos = (cell.position[0] + test_location[0] / 2, cell.position[1] + test_location[1] / 2)
                    ...
                    # Detected no cell or cell already processed
                    if detected_cell_id is None or detected_cell_id < cell_id:
                        continue
                    ...
                    if detected_cell_id in self.graph[cell_id]:
                        continue
                    ...
                    if detected_cell not in self.graph[cell_id]:
                        ...
                        if None not in terrain_mix:
                            ...
                            barrier_pos = ((cell.position[0] + detected_cell.position[0]) / 2,
                                           (cell.position[1] + detected_cell.position[1]) / 2)
    ...
    def tile_list(self):
        return [{'tile':tile.type, 'elevation': tile.elevation, 'moisture': tile.moisture, 'position': tile.position,
                 'graph': self.graph[self.map.index(tile)],
                 'resources': [self.terrain_graph.index(terrain) for terrain in self.terrain_graph if self.map.index(tile) in terrain]
                 }
                if tile is not None else {'tile':'Water', 'graph':[], 'resources':[]}
                for tile in self.map]

    def graph_list(self):
        return self.graph

    def barrier_list(self):
        return [{'type': barrier.type, 'position': barrier.position, 'orientation': barrier.orientation}
                for barrier in self.terrain_features]

    def barrier_graph_list(self):
        return self.terrain_graph

    def map_to_dict(self):
        map_dict = {
            'tiles': self.tile_list(),
            'barriers': self.barrier_list(),
            'tile_graph': self.graph_list(),
            'barrier_graph': self.barrier_graph_list()
        }
        return map_dict
